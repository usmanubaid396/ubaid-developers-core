...
/* ==========================================================================
   INTERACTIVE 3D HOLOGRAPHIC PRISM & EXPLODED BLUEPRINT ENGINE (CORE EXPERTISE)
   ========================================================================== */
const Expertise3DMatrix = () => {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      targetExplosion = Math.max(0.5, Math.min(2.5, scrollProgress * 1.5));
    };
    ...
    const animate = () => {
      ...
      prismGroup.rotation.y += (mouseX * 0.8 - prismGroup.rotation.y) * 0.08 + 0.005;
      prismGroup.rotation.x += (mouseY * 0.5 - prismGroup.rotation.x) * 0.08;

      layers.forEach((layer, idx) => {
        const explosionMultiplier = idx === activeLayer ? 1.8 : 1.0;
        layer.group.position.y += (layer.basePosY * targetExplosion * explosionMultiplier - layer.group.position.y) * 0.1;
        layer.group.rotation.y = time * (idx % 2 === 0 ? 0.3 : -0.3);
      });
  ...
  return (
    <section id="expertise" className="relative z-10 w-full border-b border-white/15 bg-transparent px-4 sm:px-8 py-24 sm:py-32">
      <div className="max-w-7xl mx-auto">
        
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-white/15 pb-6 mb-12 bg-white/[0.03] backdrop-blur-2xl px-6 rounded shadow-[0_8px_32px_0_rgba(0,0,0,0.37)] gap-4">
          <div>
          ...
          <span className="text-xs font-mono text-neutral-300 bg-white/[0.05] px-3 py-1 border border-white/20">INTERACTIVE 3D PRISM // HOVER & SCROLL</span>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
          
          <div className="lg:col-span-6 h-[450px] sm:h-[550px] border border-white/20 bg-white/[0.02] backdrop-blur-3xl relative rounded overflow-hidden shadow-[0_8px_32px_0_rgba(0,0,0,0.37)] flex items-center justify-center">
            ...
            <div className="absolute bottom-4 left-4 pointer-events-none font-mono text-[10px] text-neutral-400 bg-black/60 px-3 py-1 border border-white/10">
              [DRAG_CURSOR TO ROTATE // SCROLL TO EXPAND]
            </div>
          </div>
          ...
          <div className="lg:col-span-6 flex flex-col gap-4">
            {capabilitiesData.map((cap, idx) => (
              <div 
                ...
                className={`border p-5 sm:p-6 transition-all duration-300 cursor-pointer backdrop-blur-2xl shadow-[0_8px_32px_0_rgba(0,0,0,0.37)] flex items-start gap-4 ${
                  activeLayer === idx 
              ...
              >
                ...
                <div className="w-full">
                  <div className="flex justify-between items-center mb-1">
                    <h5 className="text-lg sm:text-xl font-black uppercase tracking-tight text-white">{cap.title}</h5>
